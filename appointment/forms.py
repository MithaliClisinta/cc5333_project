...
class CreateAppointmentForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['qualification_name'].widget.attrs.update(
            {
                'placeholder': 'Ex : level of education',
            }
        )

        self.fields['institute_name'].widget.attrs.update(
            {
                'placeholder': 'Ex : University/college',
            }
        )
...
class symptomsForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['prescription'].label = "Prescription"
        self.fields['prescribed_by'].label = "Prescribed_by"
        ...
        self.fields['prescribed_by'].widget.attrs.update(
            {
                'placeholder': 'doctor',
            }
        )
    ...
    class Meta:
        model = symptoms
        fields = ['symptoms', 'prescription', 'prescribed_by']

    def is_valid(self):
        valid = super(symptomsForm, self).is_valid()
    ...
    def save(self, commit=True):
        symptoms = super(symptomsForm, self).save(commit=False)
        if commit:
            symptoms.save()
        return symptoms    
